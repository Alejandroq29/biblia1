...
handler.use(auth).post(access('biblia-kids.chapters.manage'), async (req, res) => {
  ...
  res.status(201).json({ data: await bibliaKidsService.createChapter(parsed.data.bookId, parsed.data) });
});

handler.use(auth).get(access('biblia-kids.chapters.read'), async (req, res) => {
  const bookId = String(req.query.bookId ?? '');
  if (!bookId) return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'bookId is required' } });
